...
def extract_features(sample: Dict[str, Any], all_headers: List[str], excluded_headers: List[str]) -> Dict[str, Any]:
    ...
    features = {
        ...
        'header_order': ' '.join([header for header in headers.keys() if header.lower() in all_headers]),
...
def vectorize_features(X: List[Dict[str, Any]], all_headers: List[str]) -> tuple:
    ...
    header_order_vectorizer = CountVectorizer(analyzer='word', tokenizer=lambda x: x.split())
    ...
    feature_names = ['Header Presence'] * len(all_headers) + ['Header Order'] * header_order.shape[1] + ['Response Time', 'Status Code']
    ...
    return np.hstack([header_presence, header_order, numeric_features] + categorical_features), feature_names
