...
fn main() {
    ...
    for url_and_framework in &urls_and_frameworks {
        let (url, framework) = utils::extract_framework(url_and_framework);
        let (_, _, request_target) = http_client::parse_url(&url);
        let headers: BTreeMap<String, String> = http_client::get_default_headers(&url);

        let mut fuzzer = Fuzzer::new(methods.clone(), encoding.clone(), request_index);
        process_requests_per_method(&mut fuzzer, &url, &request_target, &headers, framework.as_deref());
        request_index = fuzzer.request_index;
    }
}

fn process_requests_per_method(fuzzer: &mut Fuzzer, url: &str, request_target: &str, headers: &BTreeMap<String, String>, framework: Option<&str>) {
    let methods = fuzzer.methods.clone();

    for method in &methods {
        process_mutated_methods(fuzzer, method, url, request_target, headers, framework);
        process_mutated_request_targets(fuzzer, method, url, request_target, headers, framework);
        process_mutated_http_versions(fuzzer, method, url, request_target, headers, framework);
        process_mutated_headers(fuzzer, method, url, request_target, framework);
    }
}

fn process_mutated_methods(fuzzer: &mut Fuzzer, method: &str, url: &str, request_target: &str, headers: &BTreeMap<String, String>, framework: Option<&str>) {
...
fn process_mutated_request_targets(fuzzer: &mut Fuzzer, method: &str, url: &str, request_target: &str, headers: &BTreeMap<String, String>, framework: Option<&str>) {
...
fn process_mutated_http_versions(fuzzer: &mut Fuzzer, method: &str, url: &str, request_target: &str, headers: &BTreeMap<String, String>, framework: Option<&str>) {
...
fn process_mutated_headers(fuzzer: &mut Fuzzer, method: &str, url: &str, request_target: &str, framework: Option<&str>) {
